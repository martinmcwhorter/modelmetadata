...
export class JSONSchema {
	...
	private Model: any;
	private key: string;
	//private properties: {[key: string]: {[name: string]: any}};
  ...
  constructor() {}
    ...
    //console.log('instance:',modelInstance);
    ...
    this.schema.required = this.required_;
		return this.schema;
	}
	...
	private required() {
		...
		if (required) this.required_.push(this.key);
	}
  ...
  private maxLength() {
   ...
   private default() {
    ...
    /*if (this.key === 'copies') 
       console.log('type:', this.key, type, "model:", 'meta:', 
       Reflect.getMetadataKeys(this.modelInstance, this.key), integer);*/
       
    		/*var message: string = Reflect.getMetadata(c.MIN_LENGTH + c.MESSAGE, this.modelInstance, this.key);
